...
TOP_LEVEL_NAME="emif_cal_one_ch"
QSYS_SIMDIR="./../../"
...
source $QSYS_SIMDIR/common/vcs_files.tcl
set files [dict merge $files [emif_cal_one_ch::get_common_design_files "$QSYS_SIMDIR"]]
...
set QSYS_SIMDIR [lindex $argv 1]
source $QSYS_SIMDIR/common/vcs_files.tcl
if {[catch {emif_cal_one_ch::get_design_files "$QSYS_SIMDIR"} errorMsg]} {
 puts "Catch error: $errorMsg"
 exit 1 
}
set files [dict create]
set files [dict merge $files [emif_cal_one_ch::get_design_files "$QSYS_SIMDIR"]]
...
run_vcs="vcs -lca -timescale=1ps/1ps -sverilog +verilog2001ext+.v $ELAB_OPTIONS $USER_DEFINED_ELAB_OPTIONS \
  ...
  $QUARTUS_INSTALL_DIR/eda/sim_lib/ctr_hssi_atoms.sv \
  $QUARTUS_INSTALL_DIR/eda/sim_lib/ctr_hssi_atoms_ncrypt.sv \
  $QUARTUS_INSTALL_DIR/eda/sim_lib/ctrb_hssi_atoms_ncrypt.sv \
  $common_design_files \
  $design_files \
  $USER_DEFINED_ELAB_OPTIONS_APPEND \
  -top $TOP_LEVEL_NAME
